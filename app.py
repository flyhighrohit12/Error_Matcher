...
def create_tfidf_vectorizer(texts: pd.Series) -> Tuple[TfidfVectorizer, np.ndarray]:
    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(texts)
    return vectorizer, tfidf_matrix

# Function to find the top matches
def find_top_matches(query: str, df: pd.DataFrame, vectorizer: TfidfVectorizer, tfidf_matrix: np.ndarray, top_n: int = 5, exclude_indices: List[int] = []) -> List[Tuple[pd.Series, float]]:
    ...
    if st.button("Get Recommendations") or (st.session_state.feedback is not None and st.session_state.last_query == user_input):
        if user_input:
            with st.spinner("Processing your request..."):
                top_matches = find_top_matches(user_input, df, vectorizer, tfidf_matrix, exclude_indices=st.session_state.excluded_indices)
